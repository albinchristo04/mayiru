...
# -*- coding: utf-8 -*-
"""
TVTap Sports Channels Fetcher - Optimized for sports only
Fetches ONLY sports TV channels from TVTap to avoid timeouts
"""
...
def is_sports_channel(channel_name):
    """
    Determine if a channel is sports-related based on name
    """
    if not channel_name:
        return False
    ...
    # Sports keywords to match
    sports_keywords = [
        'sport', 'espn', 'fox sports', 'bein', 'sky sport', 'dazn',
        'euro sport', 'nba', 'nfl', 'mlb', 'nhl', 'calcio', 'football',
        'soccer', 'tennis', 'golf', 'racing', 'motogp', 'f1', 'formula',
        'cricket', 'rugby', 'ufc', 'boxing', 'fight', 'arena',
        'premier league', 'champions', 'sports network', 'sports hd',
        'eleven sports', 'setanta', 'tsn', 'sportsnet', 'bt sport',
        'ziggo sport', 'sport1', 'sport2', 'sport tv', 'sportklub',
        'diema sport', 'arena sport', 'match tv', 'laliga', 'serie a',
        'bundesliga', 'ligue 1', 'liga', 'supersport', 'star sports',
        'sony sports', 'willow', 'tennis channel', 'golf channel'
    ]
    
    # Movie/Cinema keywords to exclude
    exclude_keywords = [
        'cinema', 'movie', 'film', 'hbo', 'cinemax', 'paramount',
        'universal', 'sony movie', 'warner', 'fox movie', 'star movie',
        'zee cinema', '&pictures', 'action movie', 'comedy movie'
    ]
    
    # Check if it's a movie channel (exclude these)
...
def fetch_sports_channels_from_api():
    """Fetch ALL channels and filter for sports only"""
            ...
            if isinstance(msg, dict) and "channels" in msg:
                ...
                for channel in channels:
                    if isinstance(channel, dict):
                        ...
                        if is_sports_channel(channel_name):
                ...
                log(f"Filtered {len(sports_channels)} sports channels")
                return sports_channels, "api"
...
def main():
    """Main function to fetch sports channels and output JSON"""
    import argparse
    
    parser = argparse.ArgumentParser(description='TVTap Sports Channels Fetcher')
    ...
    parser.add_argument('--stream-delay', type=float, default=0.3,
                       help='Delay between stream requests in seconds (default: 0.3)')
    parser.add_argument('--max-streams', type=int, default=100,
                       help='Maximum number of streams to resolve (default: 100)')
    
    args = parser.parse_args()
    
    log("Starting TVTap sports channels fetch...")
    
    channels, source = fetch_sports_channels_from_api()
    
    if not channels:
        log("No sports channels retrieved!")
        sys.exit(1)
    
    log(f"Found {len(channels)} sports channels total")
    
    # Filter by country if specified
    if args.country:
        channels = [ch for ch in channels if ch.get("country") == args.country.upper()]
        log(f"Filtered to {len(channels)} sports channels for country: {args.country}")
    ...
    # Resolve streams if requested (with max limit to avoid timeout)
    if args.resolve_streams:
        max_to_resolve = min(len(channels), args.max_streams)
        log(f"Resolving stream URLs for {max_to_resolve} channels (this may take a while)...")
        
        successful_streams = 0
        failed_streams = 0
        
        for i, channel in enumerate(channels[:max_to_resolve], 1):
            channel_id = channel.get("id")
            channel_name = channel.get("name")
            
            log(f"[{i}/{max_to_resolve}] Resolving: {channel_name} ({channel.get('country')}) (ID: {channel_id})")
        ...
        log(f"Stream resolution complete: {successful_streams} successful, {failed_streams} failed")
    ...
    output = {
        "metadata": {
            ...
            "stream_success_rate": f"{(channels_with_streams/total_channels*100):.1f}%" if args.resolve_streams and total_channels > 0 else None
        },
    ...
    log(f"Successfully fetched {total_channels} sports channels from {source}")
